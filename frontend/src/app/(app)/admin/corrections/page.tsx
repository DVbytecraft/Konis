...
import { AlertTriangle, Banknote, Package, RefreshCw, Trash2, X } from "lucide-react";
...
function CorrectionCaisse({ boutiques }: { boutiques: Lieu[] }) {
  ...
  const reset = () => {
    setLieuId(""); setMontant(""); setOperation("ajouter"); setMotif("");
        ...
        {result ? (
          <div className="space-y-3">
            <div className="rounded bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 p-4 text-sm space-y-1">
              ...
              <p><span className="text-muted-foreground">Montant :</span> {result.montant} FCFA</p>
              <p><span className="text-muted-foreground">Solde avant :</span> {result.solde_avant} FCFA</p>
              <p><span className="text-muted-foreground">Solde après :</span> <strong>{result.solde_apres} FCFA</strong></p>
    ...
    return (
      <Card>
        <CardContent className="px-4 py-5 space-y-3">
          <div className="rounded bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 p-4 text-sm space-y-1">
            <p className="font-semibold text-green-800 dark:text-green-300">Ticket annulé</p>
            <p><span className="text-muted-foreground">Numéro :</span> {result.ticket_numero}</p>
            <p><span className="text-muted-foreground">Boutique :</span> {result.lieu_nom}</p>
            <p><span className="text-muted-foreground">Montant annulé :</span> {result.montant_total} FCFA</p>
            <p><span className="text-muted-foreground">Cash restitué :</span> {result.montant_cash} FCFA</p>
            <p><span className="text-muted-foreground">Solde caisse avant :</span> {result.solde_caisse_avant} FCFA</p>
            <p><span className="text-muted-foreground">Solde caisse après :</span> <strong>{result.solde_caisse_apres} FCFA</strong></p>
            <p><span className="text-muted-foreground">Motif :</span> {result.motif}</p>
          </div>
