...
interface LigneVente { quantite: string; prix_unitaire: string }
interface Ticket {
  id: number;
  numero: string;
  date: string;
...
interface Cession { id: number; lot_nom: string; produit_nom: string; quantite_sacs: string; prix_par_sac: string; montant_cession: string; created_at: string }
interface DetailBoutique {
  lieu_id: number; lieu_nom: string; total_ventes: string; total_cessions_recues: string;
  tickets: Ticket[]; cessions_recues: Cession[];
}
...
export default function AdminDetailBoutiquePage() {
    ...
    <div className="space-y-6 min-w-0">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-lg border border-l-4 border-l-green-500 bg-green-50 dark:bg-green-950/20 p-4">
          <p className="text-xs text-muted-foreground">CA ventes</p>
        ...
        <div className="overflow-x-auto rounded-md border">
          <table className="w-full text-xs sm:text-sm min-w-[400px]">
            <thead><tr className="border-b bg-muted/40">
              <th className="text-left py-2 px-3">N°</th><th className="text-left py-2 px-3">Date</th><th className="text-right py-2 px-3">Total (FCFA)</th>
            </tr></thead>
            <tbody>
        ...
        <div className="overflow-x-auto rounded-md border">
          <table className="w-full text-xs sm:text-sm min-w-[560px]">
            <thead><tr className="border-b bg-muted/40">
              ...
              <th className="text-right py-2 px-3">Sacs</th><th className="text-right py-2 px-3">Prix/sac</th>
              <th className="text-right py-2 px-3">Montant (FCFA)</th><th className="text-left py-2 px-3">Date</th>
            </tr></thead>
            <tbody>
              {data.cessions_recues.length === 0 && <tr><td colSpan={6} className="py-4 text-center text-muted-foreground">Aucune cession.</td></tr>}
              {data.cessions_recues.map((c) => (
                <tr key={c.id} className="border-b hover:bg-muted/20">
                  <td className="py-1.5 px-3 font-mono">{c.lot_nom}</td><td className="py-1.5 px-3">{c.produit_nom}</td>
