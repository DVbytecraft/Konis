    ...
    <div className="space-y-6 min-w-0">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="rounded-lg border border-l-4 border-l-purple-500 bg-purple-50 dark:bg-purple-950/20 p-4">
          <p className="text-xs text-muted-foreground">Transferts inter-usines</p>
      ...
      <div className="scrollbar-hidden flex gap-1 border-b overflow-x-auto">
        {tabs.map(([t, label, count]) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px whitespace-nowrap transition-colors ${tab === t ? "border-green-600 text-green-600 dark:border-green-400 dark:text-green-400" : "border-transparent text-muted-foreground hover:text-foreground"}`}
          >
            {label} ({count})
          </button>
        ))}
        ...
        <div className="overflow-x-auto rounded-md border">
          <table className="w-full text-xs sm:text-sm min-w-[640px]">
            ...
            <tbody>
              ...
              {data.cessions_vers_boutiques.length > 0 && (
                <tr className="bg-green-50 dark:bg-green-950/20 font-semibold">
                  <td colSpan={5} className="py-2 px-3">Total</td>
                  <td className="py-2 px-3 text-right text-green-700 dark:text-green-400">{fmt(data.total_transferts_boutiques)}</td>
                  <td />
                </tr>
              )}
            </tbody>
      ...
      {tab === "inter_sortants" && (
        <InterUsineTable rows={data.transferts_inter_usines_sortants} total={data.total_transferts_inter_usines_sortants} direction="sortant" />
      )}

      {tab === "inter_entrants" && (
        <InterUsineTable rows={data.transferts_inter_usines_entrants} total={null} direction="entrant" />
      )}
    </div>
  );
}

function InterUsineTable({ rows, total, direction }: { rows: InterUsine[]; total: string | null; direction: "sortant" | "entrant" }) {
  return (
    <div className="overflow-x-auto rounded-md border">
      <table className="w-full text-xs sm:text-sm min-w-[680px]">
        <thead><tr className="border-b bg-muted/40">
          <th className="text-left py-2 px-3">Lot</th>
          <th className="text-left py-2 px-3">Produit</th>
          {direction === "sortant" ? <th className="text-left py-2 px-3">Destination</th> : <th className="text-left py-2 px-3">Source</th>}
          <th className="text-right py-2 px-3">Sacs</th>
          <th className="text-right py-2 px-3">Prix/sac</th>
          <th className="text-right py-2 px-3">Montant (FCFA)</th>
          <th className="text-left py-2 px-3">Date</th>
        ...
        <tbody>
          ...
          {rows.map((r) => (
            <tr key={r.id} className="border-b hover:bg-muted/20">
              <td className="py-1.5 px-3 font-mono">{r.lot_nom}</td>
              <td className="py-1.5 px-3">{r.produit_nom}</td>
              <td className="py-1.5 px-3">{direction === "sortant" ? r.usine_destination_nom : r.usine_source_nom}</td>
              <td className="py-1.5 px-3 text-right">{Number(r.quantite_sacs).toLocaleString("fr-FR")}</td>
              <td className="py-1.5 px-3 text-right">{Number(r.prix_par_sac).toLocaleString("fr-FR")}</td>
              <td className="py-1.5 px-3 text-right font-medium text-purple-700 dark:text-purple-400">{Number(r.montant_transfert).toLocaleString("fr-FR")}</td>
          ...
          {rows.length > 0 && total !== null && (
            <tr className="bg-purple-50 dark:bg-purple-950/20 font-semibold">
              <td colSpan={5} className="py-2 px-3">Total</td>
              <td className="py-2 px-3 text-right text-purple-700 dark:text-purple-400">{Number(total).toLocaleString("fr-FR")}</td>
              <td />
            </tr>
          )}
        </tbody>
